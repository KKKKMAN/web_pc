  ...
  <div class="settlementBox">
    ...
    <div class="settlementBox_top">
      ...
      <div class="settlementBox_top_dzhi">
        <div
          class="settlementBox_top_dzhi_item"
          :class="{ active: v.defaultAddress == 1 }"
          v-for="(v, i) in shippingAddressList"
          :key="i"
    ...
    <div class="settlementBox_bottom" v-for="(v, i) in orderlist" :key="i">
      ...
      <div
        class="settlementBox_b_item"
        v-for="(vv, ii) in v.productDetailsItem"
        :key="ii"
      >
        <div>
    ...
    <div class="vb_box">
      <div class="video_box">
        ...
        <span>视频确定：</span>
        <span
          >如您选择视频确认订单，在商家备货完成后，会向您发起视频通话，方便您查验商品质量。</span
        >
      </div>
    ...
    <div class="payment_box">
      <div>
        <span
          >共 <span style="color: #ea6240">{{ goodsnum }}</span> 件商品</span
        ><span class="h_span">实付款</span
        ><span style="color: #ea6240; font-size: 30px; font-weight: bold"
          >¥ {{ totalsEnd }}</span
        >
      </div>
      <div>
        <span class="h_span">配送至</span
        ><span>{{ newSite.address }} {{ newSite.addressDetails }} </span>
      </div>
      <div>
        <span class="h_span">收货人</span
        ><span>{{ newSite.contacts }} {{ newSite.phone }}</span>
      </div>
    </div>
    <div class="payment_btn">
      <Button size="large" style="font-size: 15px" @click="goBackShoppingCart"
        >返回购物车</Button
      >
      <Button
        type="primary"
        size="large"
        style="background-color: #ea6240; font-size: 15px; margin-left: 50px"
        @click="commitOrderInfo"
        >提交订单</Button
      >
    </div>
  </div>
...
export default {
  ...
  computed: {
    //最终付款
    totalsEnd() {
      return this.totals - this.couponMoney > 0
        ? this.accJian(this.totals, this.couponMoney)
        : 0;
    },
    //当前使用优惠券金额
    couponMoney() {
  ...
  methods: {
    ...
    onGetQueryActivity() {
      getQueryActivity({
        shopId: this.orderlist[0].csmShopId,
        productId: "",
      }).then((res) => {
    ...
    createNewOrderInfo() {
      this.orderlist.forEach((item) => {
        ...
        storeList.total = this.accJian(item.total, this.jian);
